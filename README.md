...
The dataset used here (``creditcard.csv``) is not included in this repository due to its size. You can download it from Kaggle and place it in the root of this project. The CSV contains 284 807 transactions from European cardholders over two days, with a ``Class`` column indicating whether a transaction is fraudulent.
